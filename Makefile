...
lint: lint-go lint-proto lint-banner
...
# golangci-lint owns the import grouping, per .golangci.yml's formatters, and it
# reports a wrong group as a lint finding. Without this line make fmt could not
# fix what make lint reported - which it could not, until a new import landed in
# the wrong group and there was nothing to run.
fmt:
...
# The one thing anything here reads a generated file for, per CLAUDE.md's Code
# generation section: the banner has to be the first line. A file whose banner
# sits lower is still *detected* as generated - measured 2026-09-19 on a banner
# at line nine, which golangci-lint skipped under its strictest setting - so
# every check that skips generated code skips it, and without this the rule
# would be about exactly the file nothing looks at.
#
# protoc-gen-go is why it can happen: it copies a .proto file's leading comment
# above its own banner.
lint-banner:
	@bad=$$(for f in $$(find proto/gen -name '*.go' -print); do \
		head -1 $$f | grep -q '^// Code generated by .* DO NOT EDIT\.$$' || echo $$f; \
	done); \
	if [ -n "$$bad" ]; then \
		echo "generated files whose first line is not the banner:"; \
		echo "$$bad"; \
		exit 1; \
	fi
